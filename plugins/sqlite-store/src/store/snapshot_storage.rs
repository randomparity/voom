...
fn gather_video_stats(conn: &rusqlite::Connection) -> Result<VideoStats> {
    let total_tracks: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM tracks WHERE track_type = 'video'",
            [],
            |row| row.get(0),
        )
    ...
    let hdr_count: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM tracks \
             WHERE track_type = 'video' AND is_hdr = 1",
            [],
            |row| row.get(0),
        )
    ...
    let vfr_count: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM tracks \
             WHERE track_type = 'video' AND is_vfr = 1",
            [],
            |row| row.get(0),
        )
    ...
    let resolution_counts = query_distribution(
        ...
        "SELECT CASE \
         ...
         WHERE track_type = 'video' AND width IS NOT NULL \
...
fn gather_audio_stats(conn: &rusqlite::Connection) -> Result<AudioStats> {
    let total_tracks: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM tracks WHERE track_type LIKE 'audio%'",
            [],
            |row| row.get(0),
        )
    ...
    let sample_rate_counts = query_distribution(
        ...
        "SELECT COALESCE(CAST(sample_rate AS TEXT), 'unknown'), COUNT(*) \
         FROM tracks WHERE track_type LIKE 'audio%' \
...
fn gather_subtitle_stats(conn: &rusqlite::Connection) -> Result<SubtitleStats> {
    let total_tracks: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM tracks \
             WHERE track_type LIKE 'subtitle%'",
            [],
            |row| row.get(0),
        )
