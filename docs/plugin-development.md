...
WASM plugins implement the `voom:plugin@0.1.0` world defined in `crates/voom-wit/wit/`:

**`world.wit`** — The plugin world:
```wit
package voom:plugin@0.1.0;
...
- **WIT-generated `PluginInfo`** — The plain record struct generated by `wit_bindgen::generate!`. Used inside `impl Guest` for `get_info()`. This is a simple struct with `name`, `version`, and `capabilities` fields.
...
- **`PluginInfoData`** — A lightweight mirror of the WIT record, re-exported at the crate root. Useful for passing plugin info across non-WIT boundaries.
